...
The primary function of this app is to help users manage the progress of their mead batches. There are currently four primary entities in the project: Recipes, Meads, Events and Readings.

The Recipe entity represents the starting details of a batch. It is freeform, so it can be as precise or general as the user likes. When a Mead entry is started from a Recipe, the Recipe details are copied to the Mead record.

The Mead entity represents the mead batch when it is first created. It has a name, a start date and an initial gravity reading. The other entities in the project (except for Recipe) all need to be related to a primary mead entity.
...
Mead batches can be tagged. Tags are simple strings that are related many-to-many to Meads and can be used for filtering and other features later.
